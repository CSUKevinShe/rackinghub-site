...
(function () {
    ...
    var BOMEngine = {
        ...
        calc: function (params) {
            ...
            //    斜撑由立柱总高度决定（约600mm间距zigzag），与横撑密度无关
            //    斜撑公式: Math.round(高度÷0.6) - 1  （已验证6m=9, 8.4m=13, 9m=14, 11m=17）
            //    D型基础2根（底150mm+顶），Z型基础6根（中间额外加固）
            //    当顶部段长 > 600mm（标准间距）时，说明顶部空缺过大，各加1根横撑封顶
            ...
            var braceSegments = Math.round(uprightHeightM / 0.6); // 斜撑分段数
            var topGapM = uprightHeightM / braceSegments; // 每段长度
            var diagonalBracesPerFace = braceSegments - 1; // 斜撑数（固定，与类型无关）
            var isZType = true; // Z型斜撑，默认true（横撑更密，刚性更强）
            var baseHorizontalBraces = isZType ? 6 : 2; // D型基础2根，Z型基础6根
            var extraBrace = topGapM > 0.6 ? 1 : 0; // 顶部段长>600mm时加1根
            var horizontalBracesPerFace = baseHorizontalBraces + extraBrace;
            var braceLenM = Math.sqrt(rackDepthM * rackDepthM + (uprightHeightM / diagonalBracesPerFace) * (uprightHeightM / diagonalBracesPerFace));
